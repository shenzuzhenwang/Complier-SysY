...
START_INLINE:
    ...
    do
    {
        if ((*it)->name != "main" && (*it)->callers.count(*it) == 0 && (*it)->fitInline(MAX_INLINE_INSTRUCTION_CNT, MAX_INLINE_WITH_POINTER_ARGUMENT))
        {
                   ...
                   shared_ptr<InvokeInstruction> &invoke, shared_ptr<Function> &toBeInline)
{
    ...
    for (int i = 0; i < invoke->params.size(); ++i)
    {
        ...
        if (invoke->params.at(i)->valueType == ValueType::INSTRUCTION && s_p_c<Instruction>(invoke->params.at(i))->resultType != L_VAL_RESULT)
        {
    ...
    {
        shared_ptr<BasicBlock> tempBlock = make_shared<BasicBlock>(callerFunc, true, it->loopDepth + callerBlock->loopDepth);
    ...
    for (auto &it : toBeInlineBlocks)
    {
        ...
        for (auto &phi : phis)
        {
            ...
            shared_ptr<PhiInstruction> newPhi = s_p_c<PhiInstruction>(findValueInMap(phiValue, funcInlineVarMap));
            ...
            for (auto &op : operands)
            {
                ...
                newPhi->operands[findBlockInMap(opBb, funcInlineBlockMap)] = findValueInMap(opVal, funcInlineVarMap);
            ...
            for (auto user : users)
            {
                ...
                {
                    if (dynamic_cast<ReturnInstruction *>(user.get()) && s_p_c<ReturnInstruction>(user)->funcType != FuncType::FUNC_INT)
                        continue;
                ...
                unordered_map<shared_ptr<Value>, shared_ptr<Value>> &copyVarMap)
{
    ...
    case InstructionType::RET:
    {
        ...
        if (retIns->funcType == FuncType::FUNC_INT)
        {
            ...
            if (retVal->valueType == ValueType::INSTRUCTION && s_p_c<Instruction>(retVal)->resultType != L_VAL_RESULT)
            {
...
shared_ptr<Value> findValueInMap(shared_ptr<Value> &value,
                                 unordered_map<shared_ptr<Value>, shared_ptr<Value>> &copyVarMap)
{
    if (copyVarMap.count(value) != 0)
        return copyVarMap.at(value);
...
shared_ptr<BasicBlock> findBlockInMap(shared_ptr<BasicBlock> &bb,
                                      unordered_map<shared_ptr<BasicBlock>, shared_ptr<BasicBlock>> &copyBlockMap)
{
    if (copyBlockMap.count(bb) != 0)
        return copyBlockMap.at(bb);
