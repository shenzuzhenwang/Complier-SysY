...
string Function::toString()
{
    ...
    for (int i = 1; i < blocks.size(); ++i)
    {
        ...
        s += "    " + getBasicBlockId(bb) + " [depth " + to_string(bb->loopDepth) + "] (id " + to_string(bb->id) + "):\n";
        s += bb->toString();
    }
    return s;
}
...
string BasicBlock::toString()
{
    ...
    for (const auto &ins : instructions)
    {
        ...
        else
        {
            ...
            s += "        " + getSsaName(ins) + " = copy " + valueAtThis + " (" + ins->caughtVarName + ") (id " + to_string(ins->id) + ")\n";
        }
    }
    return s;
}
...
{
    return "branch [condition " + getSsaName(condition) + "] [true to " + getBasicBlockId(trueBlock) + "] [false to " + getBasicBlockId(falseBlock) + "] (id " + to_string(id) + ")\n";
}
...
string InvokeInstruction::toString()
{
    ...
    {
        if (invokeType == InvokeType::GET_INT || invokeType == InvokeType::GET_CHAR || invokeType == InvokeType::GET_ARRAY)
        {
            s += getSsaName(v) + " = ";
        }
...
{
    shared_ptr<Value> v = shared_from_this();
    return getSsaName(v) + " = alloc [units " + to_string(units) + "] [bytes " + to_string(bytes) + "] (id " + to_string(id) + ")\n";
}

string StoreInstruction::toString()
{
    return "store " + getSsaName(value) + " to " + getSsaName(address) + " [offset " + getSsaName(offset) + "] (id " + to_string(id) + ")\n";
}
