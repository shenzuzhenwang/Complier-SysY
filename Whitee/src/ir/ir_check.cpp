...
bool irCheck(const shared_ptr<Module> &module)
{
    ...
    {
        for (auto &user : cst->users)
            if (!user->valid)
                irError("global constant has invalid users.");
        if (irUserCheck && cst->users.empty())
            irError("constant has no users.");
    }
    for (auto &glb : module->globalVariables)
    {
        for (auto &user : glb->users)
            if (!user->valid)
                irError("global variable has invalid users.");
        if (irUserCheck && glb->users.empty())
            irError("global variable has no users.");
    }
    for (auto &str : module->globalStrings)
    {
        for (auto &user : str->users)
            if (!user->valid)
                irError("global string has invalid users.");
        if (irUserCheck && str->users.empty())
            irError("string has no users.");
    }
...
void basicBlockCheck(const shared_ptr<BasicBlock> &bb)
{
    ...
    if (!bb->instructions.empty() && (*(bb->instructions.end() - 1))->type != InstructionType::JMP && (*(bb->instructions.end() - 1))->type != InstructionType::BR && (*(bb->instructions.end() - 1))->type != InstructionType::RET)
    {
...
void instructionCheck(const shared_ptr<Instruction> &ins)
{
    ...
    {
        if (!user->valid)
            irError("instruction users contain invalid value in function " + ins->block->function->name + " block " + to_string(ins->block->id) + ".");
    }
    switch (ins->type)
    {
    ...
    case InstructionType::BR:
    {
        ...
        else if (inst->trueBlock->predecessors.count(inst->block) == 0 || inst->block->successors.count(inst->trueBlock) == 0)
        ...
        else if (inst->falseBlock->predecessors.count(inst->block) == 0 || inst->block->successors.count(inst->falseBlock) == 0)
    ...
    case InstructionType::JMP:
    {
        ...
        else if (inst->targetBlock->predecessors.count(inst->block) == 0 || inst->block->successors.count(inst->targetBlock) == 0)
